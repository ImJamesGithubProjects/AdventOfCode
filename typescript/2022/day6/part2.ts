...
// Given a string of length 14, are all the characters unique?
...
function position_of_marker(s: string): number {
    for (let i = 14; i < input.length; i++) {
        if (is_marker(s.slice(i - 14, i))) return i;
    }
...
function position_of_marker_rec(s: string): number {
    ...
    return position_of_marker_rec2(s, 14);
}
